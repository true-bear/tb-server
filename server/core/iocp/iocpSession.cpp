...
IocpSession::~IocpSession() = default;
...
void IocpSession::DisconnectFinish()
{
...
bool IocpSession::AcceptReady(const SOCKET& listenSock, const int uID)
{
...
bool IocpSession::AcceptFinish(const SOCKET& listenSocket)
{
...
bool IocpSession::RecvPacket(unsigned long ioSize)
{
...
char* IocpSession::GetRecvOverlappedBuffer() const
{
	return mRecvOverEx.mWsaBuf.buf;
}

bool IocpSession::SendPacket(const char* data, unsigned long long packetSize)
{
